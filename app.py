...
def create_brochure_pdf(car_details, specs):
    """Engine to generate a professional, branded PDF natively in Python."""
    pdf = FPDF()
    pdf.add_page()
    
    # Corporate Header
    pdf.set_font('Arial', 'B', 16)
    pdf.set_text_color(0, 51, 102)  # Official BMW Dark Blue
    pdf.cell(0, 10, 'BMW SANDTON', 0, 1, 'L')
    pdf.set_font('Arial', 'I', 10)
    pdf.set_text_color(100, 100, 100)
    pdf.cell(0, 5, 'OFFICIAL VEHICLE SPECIFICATION SHEET', 0, 1, 'L')
    ...
    pdf.cell(0, 10, car_details['desc'], 0, 1, 'L')
    
    # VSB & VIN Identification
    pdf.set_font('Arial', '', 11)
    pdf.set_text_color(80, 80, 80)
    pdf.cell(0, 6, f"VSB Number: {car_details['vsb']}   |   VIN / Chassis: {car_details['vin']}", 0, 1, 'L')
    pdf.ln(5)
    
    # Value Proposition
    pdf.set_font('Arial', 'B', 16)
    pdf.set_text_color(0, 51, 102)
    pdf.cell(0, 10, f"Investment Value: {car_details['price']}", 0, 1, 'L')
    pdf.ln(10)
    
    # Extracted Technical Specifications
    pdf.set_font('Arial', 'B', 14)
    pdf.set_text_color(0, 0, 0)
    pdf.cell(0, 10, 'Technical Specifications', 'B', 1, 'L')
    pdf.ln(5)
    
    for k, v in specs.items():
        pdf.set_font('Arial', 'B', 11)
        pdf.set_text_color(50, 50, 50)
        pdf.cell(50, 8, f"{k}:", 0, 0, 'L')
        pdf.set_font('Arial', '', 11)
        pdf.set_text_color(0, 0, 0)
        pdf.cell(0, 8, str(v), 0, 1, 'L')
        
    pdf.ln(10)
    
    # Quality Assurance Footer
    pdf.set_font('Arial', 'B', 14)
    pdf.cell(0, 10, 'Dealership Verification', 'B', 1, 'L')
    pdf.ln(5)
    pdf.set_font('Arial', '', 11)
    pdf.multi_cell(0, 6, "This vehicle has undergone a comprehensive multi-point quality check by certified BMW technicians. It includes a full franchise service history, remaining motorplan balance, and comes with guaranteed clear title and verified mileage.")
    
    # Base Document Footer
    pdf.set_y(-20)
    pdf.set_font('Arial', 'I', 8)
    pdf.set_text_color(150, 150, 150)
    pdf.cell(0, 10, 'BMW Sandton | Generated by Digital Dealership System | Confidential & Proprietary', 0, 0, 'C')
    
    # Output robustly based on fpdf version
    ...
    # =========================================================
        ...
        with tab3:
            ...
            if not df_live_stock.empty:
                ...
                # 🟢 NEW FEATURE: 1-Click Digital Spec Sheet Generator (Brochure Studio)
                st.markdown("---")
                ...
                if FPDF_AVAILABLE:
                    ...
                    if selected_brochure != "Select a Vehicle...":
                        ...
                        # Simulated Internet Spec Generation
                        ...
                        with col_b2:
                            ...
                            try:
                                pdf_bytes = create_brochure_pdf(car_details, specs)
                                st.markdown("<br>", unsafe_allow_html=True)
                                st.download_button(
                                    label="📥 DOWNLOAD PROFESSIONAL PDF BROCHURE",
                                    data=pdf_bytes,
                                    file_name=f"BMW_Spec_Sheet_{sel_vsb}.pdf",
                                    mime="application/pdf"
                                )
                            except Exception as e:
                                st.error(f"Failed to compile PDF document. Error details: {e}")
                else:
                    st.error("⚠️ The `fpdf` library is not installed. Please add `fpdf` to your `requirements.txt` file to enable the Brochure Studio.")
