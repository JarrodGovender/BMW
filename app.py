...
def init_db():
    ...
    # Drop the old structural tables to ensure a completely clean slate
    c.execute("DROP TABLE IF EXISTS leads")
    c.execute("DROP TABLE IF EXISTS individual_leads")
    
    # 1. System structural tables created safely
    ...
    # 2. Pipeline Table A: Corporate Fleet Leads (B2B) - 13 Columns
    ...
    # 3. Pipeline Table B: Individual Luxury Leads (B2C) - 12 Columns
...
# THE AUTO-INGEST ENGINE: Background processing pipeline simulating live JSE SENS scraping
def sync_live_jse_sens():
    ...
    # 1. Automated Feed Vector A: Live JSE Corporate Fleet Signals (Entire Gauteng Region)
    ...
    # 2. Automated Feed Vector B: Live JSE Executive Appointments (Individual Leads including Middle Management)
    ...
    # Safe insertion wrappers to avoid duplication entries during page state re-runs
...
# Execute infrastructure preparation steps
init_db()
sync_live_jse_sens()
