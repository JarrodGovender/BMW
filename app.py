...
def create_brochure_excel(car_details, specs):
    ...
    with pd.ExcelWriter(excel_buffer, engine='xlsxwriter') as writer:
        ...
        ws.merge_range('A1:B2', 'PHASE V MOTOR INVESTMENTS - OFFICIAL DIGITAL SPEC SHEET', title_fmt)
...
if st.session_state['authenticated']:
    ...
    if st.session_state['page_view'] == 'settings':
        ...
        if st.session_state['role'] == 'property_manager':
            ...
            try:
                sites_res = supabase.table("property_sites").select("*").order("id", desc=False).execute()
                        ...
                        for status, col, icon in [("Open", col_open, "⚪"), ("In Progress", col_prog, "🔵"), ("Completed", col_arch, "🏁")]:
                            with col:
                                ...
                                if subset.empty:
                                    ...
                                    subset = subset.sort_values(by='created_at', ascending=False)
                                        ...
                                        with st.expander(f"{pri_emoji} {task_row['task_title']}"):
                                            ...
                                            if notes_data:
                                                for note in notes_data:
                                                    bg, border = container_bg, text_color
                                                    if "(Director)" in note['author_name']: bg, border = ("#2b1c1c" if theme == 'Dark' else "#ffe6e6"), "#ff4b4b"
                                                    st.markdown(f"<div style='background-color:{bg}; padding:8px; margin-bottom:5px; border-left:3px solid {border};'><small><b>{note['author_name']}</b> - {str(note['created_at']).split('T')[0]}</small><br>{note['note_text']}</div>", unsafe_allow_html=True)
                                            else:
                                            ...
                                            if st.button("POST NOTE", key=f"note_btn_{task_id}"):
                                                if new_note:
                                                    supabase.table("task_notes").insert({"task_id": task_id, "note_text": new_note, "author_name": st.session_state['name']}).execute()
                                                    safe_rerun()

                # Archive Tab (Last Tab)
                with site_tabs[-1]:
                    ...
                    if df_archived.empty:
                        ...
                        for _, a_row in df_archived.iterrows():
                            t_id = a_row['id']
                            p_icon = str(a_row.get('priority', '🟡 Medium')).split()[0]
                            with st.expander(f"🗃️ [{str(a_row.get('site_name', 'Unknown')).upper()}] {p_icon} {a_row['task_title']}"):
                                ...
                                st.caption(f"Created by: {a_row.get('created_by', 'System')} | {str(a_row['created_at']).split('T')[0]}")
                                
                                if st.button("⏪ UNARCHIVE TASK", key=f"unarch_{t_id}"):
                                    supabase.table("site_tasks").update({"is_archived": False}).eq("id", t_id).execute()
                                    safe_rerun()
        ...
        # ====================================================================
            ...
            df_s = pd.DataFrame(all_sites) if all_sites else pd.DataFrame()
            df_t = pd.DataFrame(all_tasks) if all_tasks else pd.DataFrame()
            
            if not df_t.empty and 'is_archived' not in df_t.columns:
                df_t['is_archived'] = False

            with dir_tab1:
                st.markdown("### 📈 PHASE V - EXECUTIVE PORTFOLIO DASHBOARD")
                st.caption("High-level operational overview across all physical properties. (Excludes Archived Tasks)")
                ...
                if df_s.empty or df_t.empty:
                    ...
                    if drill_df.empty:
                        st.success("No active tasks match the current executive filter parameters.")
                    else:
                        ...
                        for _, task_row in drill_df.iterrows():
                            ...
                            with st.expander(f"{s_icon} [{str(task_row.get('site_name','Unknown')).upper()}] {p_icon} {task_row['task_title']}"):
                                ...
                                if st.button("ISSUE DIRECTIVE", key=f"dir_note_btn_{task_id}"):
                                    if new_dir_note:
                                        supabase.table("task_notes").insert({"task_id": int(task_id), "note_text": new_dir_note, "author_name": f"{st.session_state['name']} (Director)"}).execute()
                                        st.success("Directive published to task timeline."); safe_rerun()

            with dir_tab2:
                st.markdown("### 🗄️ EXECUTIVE ARCHIVE LEDGER")
                st.caption("Review completed and archived tasks, or pull them back to active status.")
                    ...
                    if not df_arch.empty:
                        ...
                        for _, a_row in df_arch.iterrows():
                            ...
                            with st.expander(f"🗃️ [{str(a_row.get('site_name', 'Unknown')).upper()}] {p_icon} {a_row['task_title']}"):
                                ...
                                if st.button("⏪ UNARCHIVE TASK", key=f"dir_unarch_{t_id}"):
                                    supabase.table("site_tasks").update({"is_archived": False}).eq("id", t_id).execute()
                                    safe_rerun()
                                ...
                                if notes_data:
                                    for note in notes_data:
                                        ...
                                        if "(Director)" in note['author_name']: bg, border = ("#2b1c1c" if theme == 'Dark' else "#ffe6e6"), "#ff4b4b"
                                        st.markdown(f"<div style='background-color:{bg}; padding:8px; margin-bottom:5px; border-left:3px solid {border};'><small><b>{note['author_name']}</b> - {str(note['created_at']).split('T')[0]}</small><br>{note['note_text']}</div>", unsafe_allow_html=True)
                                else: st.caption("No historical notes.")
                                
                                new_dir_note = st.text_area("APPEND NOTE TO ARCHIVE", key=f"arch_note_in_{t_id}")
                                if st.button("POST TO ARCHIVE", key=f"arch_note_btn_{t_id}"):
                                    if new_dir_note:
                                        supabase.table("task_notes").insert({"task_id": int(t_id), "note_text": new_dir_note, "author_name": f"{st.session_state['name']} (Director)"}).execute()
                                        safe_rerun()
                    else: st.info("No tasks in the global archive.")
                else: st.info("System is currently empty.")
        ...
        # ====================================================================
            ...
            if IS_MANAGEMENT:
                tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["🔥 AVAILABLE DAILY FEED", "💼 MY CLAIMED ACCOUNTS", "🚗 USED CAR STOCKROOM", "💼 PIPELINE TRACKER", "📦 ARCHIVED DELIVERIES", "📊 COMMAND OVERVIEW", "💰 F&I DESK"])
            else:
                tab1, tab2, tab3, tab4, tab5 = st.tabs(["🔥 AVAILABLE DAILY FEED", "💼 MY CLAIMED ACCOUNTS", "🚗 USED CAR STOCKROOM", "💼 PIPELINE TRACKER", "📦 ARCHIVED DELIVERIES"])
            ...
            with tab1:
                ...
                if lead_section == "🏢 Corporate Fleet (B2B)":
                    try: res = supabase.table("leads").select("*").eq("status", "Unassigned").eq("lead_date", filter_date_str).order("score", desc=True).execute().data
                    except: res = []
                    ...
                    try: res = supabase.table("individual_leads").select("*").eq("status", "Unassigned").eq("lead_date", filter_date_str).order("score", desc=True).execute().data
                    except: res = []
                    df = pd.DataFrame(res) if res else pd.DataFrame()
                    ...
                    try: res = supabase.table("tender_leads").select("*").eq("status", "Unassigned").eq("lead_date", filter_date_str).order("score", desc=True).execute().data
                    except: res = []
                    df = pd.DataFrame(res) if res else pd.DataFrame()
            ...
            with tab2:
                try: my_corp_res = supabase.table("leads").select("*").eq("assigned_to", st.session_state['user']).eq("status", "Claimed").order("id", desc=True).execute().data
                except: my_corp_res = []
                try: my_ind_res = supabase.table("individual_leads").select("*").eq("assigned_to", st.session_state['user']).eq("status", "Claimed").order("id", desc=True).execute().data
                except: my_ind_res = []
                try: my_tend_res = supabase.table("tender_leads").select("*").eq("assigned_to", st.session_state['user']).eq("status", "Claimed").order("id", desc=True).execute().data
                except: my_tend_res = []
                
                st.markdown("### 🏢 CLAIMED CORPORATE FLEET ACCOUNTS")
                if not my_corp_res: st.caption("No active corporate fleet claims linked to your profile.")
                else:
                    for row in my_corp_res:
                ...
                if not my_ind_res: st.caption("No active individual accounts claimed.")
                else:
                    for row in my_ind_res:
                ...
                if not my_tend_res: st.caption("No active government tender claims currently flagged.")
                else:
                    for row in my_tend_res:
            ...
            with tab3:
                ...
                if SHOW_UNENCUMBERED: 
                    stock_tab_main, stock_tab_demo, stock_tab_unenc = st.tabs(["🌍 MASTER USED INVENTORY", "🔵 DEMO VEHICLES", "🟢 UNENCUMBERED ASSETS"])
                else: 
                    stock_tab_main, stock_tab_demo = st.tabs(["🌍 MASTER USED INVENTORY", "🔵 DEMO VEHICLES"])

                # --- 1. ADMIN CONSOLE ISOLATED OUTSIDE THE EMPTY CHECK ---
                with stock_tab_main:
                    if IS_MANAGEMENT and st.session_state['role'] == 'finance_admin':
                        with st.expander("🛠️ ADMIN CONSOLE: MASTER INVENTORY MANAGEMENT", expanded=False):
                            st.markdown("#### 1. Paste Daily Spreadsheet Data (USED STOCK)")
                            raw_paste_data = st.text_area("PASTE RAW DATA ROWS HERE", height=150, placeholder="Franchise: B - BMW\n109237\tX4 xDrive20d Sport A...", key="used_paste")
                            
                            if st.button("PROCESS AND OVERWRITE USED INVENTORY", key="process_stock_paste_btn"):
                            ...
                            st.markdown("---")
                            st.markdown("#### 2. Run Global Floorplan Recon (Used & Demo)")
                    ...
                    if df_live_stock.empty:
                        st.info("No vehicles currently recorded in stock. Use the Admin Console to initialize the inventory.")
                    else:
                        ...
                        if 'stock_type' not in df_live_stock.columns: df_live_stock['stock_type'] = "Used"
                        
                        df_live_stock['stock_type'] = df_live_stock['stock_type'].fillna("Used")
                        ...
                        df_live_stock["location"] = df_live_stock["location"].astype(str).str.strip()
                        df_live_stock["FRANCHISE DIVISION"] = df_live_stock.apply(lambda x: f"{x['location']} (DEMO)" if str(x.get('stock_type', 'Used')) == 'Demo' else x['location'], axis=1)
                                    ...
                                    if target_email:
                                        try:
                                            ...
                                            with st.spinner("Rendering styled Excel templates and connecting to secure mail server..."):
                                                ...
                                                # Add Grand Totals for Master Stockbook    
                                                ...
                                                with pd.ExcelWriter(excel_buffer, engine='xlsxwriter') as writer:
                                                    ...
                                                    row_cursor = 1
                                                    row_cursor = draw_executive_table(ws_exec, df_sum_export, row_cursor, 0, "DEALERSHIP USED CAR STOCK SUMMARY OVERVIEW", [text_format, num_format, curr_format]) + 3
                                                    row_cursor = draw_executive_table(ws_exec, df_prov_export, row_cursor, 0, "DEALERSHIP VEHICLE AGING PROVISION MATRIX", [text_format, curr_format, curr_format, curr_format, curr_format, curr_format]) + 3
                                                    row_cursor = draw_executive_table(ws_exec, df_unenc_export, row_cursor, 0, "DEALERSHIP UNENCUMBERED STOCK MATRIX", [text_format, num_format, curr_format]) + 3
                        ...
                        for franchise in loop_franchises:
                            if franchise.strip() == "LHP" or not franchise.strip(): continue
                            franchise_df = used_filtered[used_filtered["FRANCHISE DIVISION"] == franchise].copy()
                ...
                with stock_tab_demo:
                    if IS_MANAGEMENT and st.session_state['role'] == 'finance_admin':
                        with st.expander("🛠️ ADMIN CONSOLE: DEMO INVENTORY UPLOAD", expanded=False):
                            st.markdown("#### 1. Paste Daily Spreadsheet Data (DEMO STOCK ONLY)")
                            raw_paste_data_demo = st.text_area("PASTE RAW DATA ROWS HERE", height=150, placeholder="Franchise: B - BMW\n109237\tX4 xDrive20d Sport A...", key="demo_paste")
                            
                            if st.button("PROCESS AND OVERWRITE DEMO INVENTORY", key="process_demo_paste_btn"):
                                if raw_paste_data_demo.strip():
                                    try:
                                        try:
                                            mem_res = supabase.table("used_car_stock").select("vsb_no, comments").execute()
                                            comment_memory = {str(row['vsb_no']).strip(): row.get('comments', '') for row in mem_res.data} if mem_res.data else {}
                                        ...
                                        lines = raw_paste_data_demo.split('\n')
                                        records_processed = 0; current_franchise = "General Demo Stock"
                                        ...
                                        for line in lines:
                                            ...
                                            if len(parts) >= 2 and parts[0].strip().isdigit():
                                                ...
                                                try: supabase.table("used_car_stock").upsert({"vsb_no": vsb, "description": desc, "into_stock": into_stk, "days_in_stock": days, "total_value": val, "location": current_franchise.strip(), "chassis_no": chassis, "floorplan_status": "⚪ PENDING RECON", "comments": mem_comment, "stock_type": "Demo"}).execute()
                                                except Exception: supabase.table("used_car_stock").upsert({"vsb_no": vsb, "description": desc, "into_stock": into_stk, "days_in_stock": days, "total_value": val, "location": current_franchise.strip(), "chassis_no": chassis}).execute()
                                                records_processed += 1
                                        st.success(f"🎉 Demo Stock refreshed successfully. {records_processed} units inserted."); safe_rerun()
                                    except Exception as parse_ex: st.error(f"Data processing failed: {str(parse_ex)}")
                                else: st.warning("Please populate the data terminal before submitting.")
                    ...
                    if df_live_stock.empty:
                        ...
                        if demo_filtered.empty:
                            ...
                            loop_demo_franchises = sorted(list(demo_filtered["FRANCHISE DIVISION"].unique()))
                            for franchise in loop_demo_franchises:
                                if franchise.strip() == "LHP" or not franchise.strip(): continue
                                franchise_df = demo_filtered[demo_filtered["FRANCHISE DIVISION"] == franchise].copy()
                                ...
                                if not franchise_df.empty:
                                    ...
                                    for _, row in franchise_df.iterrows():
                                        ...
                                        row_dict = {
                                            "VSB NUMBER": row.get("VSB NUMBER", ""), "VEHICLE DESCRIPTION": row.get("VEHICLE DESCRIPTION", ""),
                                            "INTO STOCK DATE": row.get("INTO STOCK DATE", ""), "DAYS ON FLOOR": f"{days} Days"
                                        }
                                        if IS_MANAGEMENT: row_dict["FP STATUS"] = row.get("FP STATUS", "")
                                        row_dict["CAPITAL VAL (ZAR)"] = f"R {float(row.get('CAPITAL VAL (ZAR)', 0)):,.2f}"
                                        render_rows.append(row_dict)
                                            ...
                                            with st.spinner("Formatting Unencumbered Matrix..."):
                                                try:
                                                    ...
                                                    with pd.ExcelWriter(excel_buffer, engine='xlsxwriter') as writer:
                                                        ...
                                                        gold_fmt = workbook.add_format({'bg_color': '#FFC000', 'border': 1, 'valign': 'vcenter'})
                                                        ...
                                                        for c_name, c_df in custom_exposure:
                                                            totv = float(c_df["CAPITAL VAL (ZAR)"].sum())
                                                            unencv = float(c_df[c_df["FP STATUS"] == "🟢 UNENCUMBERED"]["CAPITAL VAL (ZAR)"].sum())
                                                            if totv > 0:
                                                                exp_data.append([f"Total {c_name}", totv])
                                                                exp_data.append([f"Total {c_name} unencumbered", unencv])
                                                                gs += totv
                                                                gu += unencv
                                                        ...
                                                        ri = 1
                                                        ws_exec.merge_range(ri, 0, ri, 1, "DEALERSHIP CAPITAL EXPOSURE", t_fmt)
                                                        ws_exec.set_row(ri, 30)
                                                        ri += 2
                                                        for item in exp_data:
                                                            ...
                                                            fl = gold_fmt if is_u else txt_fmt
                                                            fv = gold_cur if is_u else c_fmt
                                                            
                                                            if lbl == "Total unencumbered":
                                                                fl = workbook.add_format({'bold': True, 'bg_color': '#FFC000', 'border': 1})
                                                                fv = workbook.add_format({'bold': True, 'bg_color': '#FFC000', 'border': 1, 'num_format': 'R #,##0.00'})
                                                                ws_exec.write(ri, 2, gu/gs if gs else 0, workbook.add_format({'bold': True, 'font_size': 14, 'font_color': 'red', 'bg_color': '#FFFF00', 'align': 'center', 'valign': 'vcenter', 'num_format': '0%'}))
                                                                
                                                            ws_exec.write(ri, 0, lbl, fl)
                                                            if val != "": ws_exec.write_number(ri, 1, val, fv)
                                                            ri += 1
                                                            
                                                        rstart = ri + 2
                                                        ws_exec.merge_range(rstart, 0, rstart, 2, "FLOORPLAN RATIO", t_fmt)
                                                        ws_exec.set_row(rstart, 30); ws_exec.set_row(rstart+1, 35)
                                                        ws_exec.write(rstart+1, 0, "FINANCE STATUS", h_fmt); ws_exec.write(rstart+1, 1, "TOTAL UNITS", h_fmt); ws_exec.write(rstart+1, 2, "% OF TOTAL", h_fmt)
                                                        
                                                        totu = len(df_live_stock)
                                                        uu = len(unenc_df)
                                                        ...
                                                        for fran in sorted(list(unenc_df["FRANCHISE DIVISION"].unique())):
                                                            ...
                                                            safe_sheet_name = str(fran).replace('/', '-').replace('\\', '-').replace('?', '').replace('*', '').replace('[', '').replace(']', '')[:31]
                                                    ...
                                                    msg.add_attachment(excel_buffer.getvalue(), maintype='application', subtype='vnd.openxmlformats-officedocument.spreadsheetml.sheet', filename=f"PhaseV_Unencumbered_{datetime.now(SAST).strftime('%Y%m%d')}.xlsx")
            ...
            with tab4:
                ...
                if st.session_state['role'] in MANAGEMENT_ROLES:
                    ...
                    try: res = supabase.table("sales_pipeline").select("*").neq("stage", "Delivered").order("id", desc=True).execute().data
                    except: res = []
                else:
                    st.markdown("#### 👤 MY ACTIVE DEALS")
                    try: res = supabase.table("sales_pipeline").select("*").eq("salesperson_username", st.session_state['user']).neq("stage", "Delivered").order("id", desc=True).execute().data
                    except: res = []
                
                df_pipeline = pd.DataFrame(res) if res else pd.DataFrame()
            ...
            with tab5:
                ...
                if IS_MANAGEMENT:
                    try: arc_res = supabase.table("sales_pipeline").select("*").eq("stage", "Delivered").execute().data
                    except: arc_res = []
                else:
                    try: arc_res = supabase.table("sales_pipeline").select("*").eq("salesperson_username", st.session_state['user']).eq("stage", "Delivered").execute().data
                    except: arc_res = []
                    
                df_archive = pd.DataFrame(arc_res) if arc_res else pd.DataFrame()
            ...
            if IS_MANAGEMENT:
                with tab6:
                    ...
                    try:
                        try: raw_res = supabase.table("used_car_stock").select("total_value, days_in_stock, location, floorplan_status, stock_type").execute().data
                        except: raw_res = supabase.table("used_car_stock").select("total_value, days_in_stock, location").execute().data
                        df_summary = pd.DataFrame(raw_res) if raw_res else pd.DataFrame()
                    ...
                    if not df_summary.empty and 'stock_type' not in df_summary.columns: df_summary['stock_type'] = "Used"
                    
                    if not df_summary.empty:
                        df_summary["location"] = df_summary["location"].astype(str).str.strip()
                        df_summary["FRANCHISE DIVISION"] = df_summary.apply(lambda x: f"{x['location']} (DEMO)" if str(x.get('stock_type', 'Used')) == 'Demo' else x['location'], axis=1)
                    ...
                    for cat_name, mask, is_demo in categories_def:
                        ...
                        ts+=units; tv+=val_sum; tp25+=p_2_5; tp50+=p_5_0; tp75+=p_7_5; tp100+=p_10_0; tpt+=p_total; tuu+=unenc_units; tuv+=unenc_val
                    ...
                    summary_matrix_data.append({"STOCK DIVISION": "GRAND TOTAL", "UNITS ON HAND": f"{ts:,.0f}", "PORTFOLIO INVESTMENT VALUE": f"R {tv:,.2f}"})
                    provision_rows.append({"STOCK DIVISION": "GRAND TOTAL", "2.5% (30-60 Days)": f"R {tp25:,.2f}", "5.0% (61-90 Days)": f"R {tp50:,.2f}", "7.5% (91-120 Days)": f"R {tp75:,.2f}", "10.0% (121+ Days)": f"R {tp100:,.2f}", "TOTAL PROVISION": f"R {tpt:,.2f}"})
                    unencumbered_matrix_data.append({"STOCK DIVISION": "GRAND TOTAL", "NO. OF UNENCUMBERED UNITS": f"{tuu:,.0f}", "UNENCUMBERED CAPITAL VALUE (ZAR)": f"R {tuv:,.2f}"})
...
Submitted by: {st.session_state['name']} (Finance/Admin)
    ...
    with gate_col2:
        ...
        with col_logo2:
            try:
                st.image(get_local_img("PHASEV.png", ""), use_container_width=True)
            except Exception:
                st.markdown("<h2 style='text-align: center;'>PHASE V MOTOR INVESTMENTS</h2>", unsafe_allow_html=True)
